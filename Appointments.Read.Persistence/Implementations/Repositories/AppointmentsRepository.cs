...
namespace Appointments.Read.Persistence.Implementations.Repositories
{
    public class AppointmentsRepository : Repository<Appointment>, IAppointmentsRepository
    {
        ...
        public async Task<IEnumerable<TimeSlotAppointmentDTO>> GetAppointmentsAsync(DateOnly date, IEnumerable<Guid> doctors)
        {
            return await DbSet
                .AsNoTracking()
                .Where(a => a.Date.Equals(date) && doctors.Any(x => x.Equals(a.DoctorId)))
